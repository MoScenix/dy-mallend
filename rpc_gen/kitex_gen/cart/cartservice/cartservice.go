...
func setItemHandler(ctx context.Context, handler interface{}, arg, result interface{}) error {
	switch s := arg.(type) {
	case *streaming.Args:
		st := s.Stream
		req := new(cart.SetItemReq)
		if err := st.RecvMsg(req); err != nil {
			return err
		}
		resp, err := handler.(cart.CartService).SetItem(ctx, req)
		if err != nil {
			return err
		}
		return st.SendMsg(resp)
	case *SetItemArgs:
		success, err := handler.(cart.CartService).SetItem(ctx, s.Req)
		if err != nil {
			return err
		}
		realResult := result.(*SetItemResult)
		realResult.Success = success
		return nil
	default:
		return errInvalidMessageType
...
func newSetItemArgs() interface{} {
	return &SetItemArgs{}
}

func newSetItemResult() interface{} {
	return &SetItemResult{}
}

type SetItemArgs struct {
	Req *cart.SetItemReq
}

func (p *SetItemArgs) FastRead(buf []byte, _type int8, number int32) (n int, err error) {
	if !p.IsSetReq() {
		p.Req = new(cart.SetItemReq)
	}
	return p.Req.FastRead(buf, _type, number)
}

func (p *SetItemArgs) FastWrite(buf []byte) (n int) {
	if !p.IsSetReq() {
		return 0
	}
	return p.Req.FastWrite(buf)
...
func (p *SetItemArgs) Marshal(out []byte) ([]byte, error) {
	if !p.IsSetReq() {
		return out, nil
	}
	return proto.Marshal(p.Req)
...
func (p *SetItemArgs) Unmarshal(in []byte) error {
	msg := new(cart.SetItemReq)
	if err := proto.Unmarshal(in, msg); err != nil {
		return err
	}
	p.Req = msg
...
var SetItemArgs_Req_DEFAULT *cart.SetItemReq

func (p *SetItemArgs) GetReq() *cart.SetItemReq {
	if !p.IsSetReq() {
		return SetItemArgs_Req_DEFAULT
	}
	return p.Req
}

func (p *SetItemArgs) IsSetReq() bool {
	return p.Req != nil
}

func (p *SetItemArgs) GetFirstArgument() interface{} {
	return p.Req
}

type SetItemResult struct {
	Success *cart.SetItemResp
}

var SetItemResult_Success_DEFAULT *cart.SetItemResp

func (p *SetItemResult) FastRead(buf []byte, _type int8, number int32) (n int, err error) {
	if !p.IsSetSuccess() {
		p.Success = new(cart.SetItemResp)
	}
	return p.Success.FastRead(buf, _type, number)
}

func (p *SetItemResult) FastWrite(buf []byte) (n int) {
	if !p.IsSetSuccess() {
		return 0
	}
	return p.Success.FastWrite(buf)
...
func (p *SetItemResult) Size() (n int) {
	if !p.IsSetSuccess() {
		return 0
	}
	return p.Success.Size()
...
func (p *SetItemResult) Marshal(out []byte) ([]byte, error) {
	if !p.IsSetSuccess() {
		return out, nil
	}
	return proto.Marshal(p.Success)
...
func (p *SetItemResult) Unmarshal(in []byte) error {
	msg := new(cart.SetItemResp)
	if err := proto.Unmarshal(in, msg); err != nil {
		return err
	}
	p.Success = msg
...
func (p *SetItemResult) GetSuccess() *cart.SetItemResp {
	if !p.IsSetSuccess() {
		return SetItemResult_Success_DEFAULT
	}
	return p.Success
}

func (p *SetItemResult) SetSuccess(x interface{}) {
	p.Success = x.(*cart.SetItemResp)
}

func (p *SetItemResult) IsSetSuccess() bool {
	return p.Success != nil
}

func (p *SetItemResult) GetResult() interface{} {
	return p.Success
}
...
func (p *kClient) SetItem(ctx context.Context, Req *cart.SetItemReq) (r *cart.SetItemResp, err error) {
	var _args SetItemArgs
	_args.Req = Req
	var _result SetItemResult
	if err = p.c.Call(ctx, "SetItem", &_args, &_result); err != nil {
		return
	}
	return _result.GetSuccess(), nil
}
